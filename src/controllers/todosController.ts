...
const selectAll = async (req: Request, res: Response) => {
    ...
    (!error) ? res.send(JSON.stringify(data)) : res.status(500).send(new RequestError(RequestErrorCodes.DATABASE_ERROR, "An error has transpired during the data processing procedure."))
}
...
const deleteById = async (req: Request, res: Response) => {
    ...
    (!error) ? res.send(JSON.stringify({ error: false })) : res.status(500).send(new RequestError(RequestErrorCodes.DATABASE_ERROR, "An error has transpired during the data processing procedure."))
}
